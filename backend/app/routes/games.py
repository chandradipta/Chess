...
async def create_game(payload: GameCreate, current_user = Depends(get_current_user_optional)):
    async with AsyncSessionLocal() as session:
        ...
        await game_manager.create_session_from_db(game.id, ai_side=game.ai_side, ai_difficulty=game.ai_difficulty)
        return GameOut(
            game_id=game.id,
            ai=(game.ai == "true"),
...
async def get_game_pgn(game_id: int):
    async with AsyncSessionLocal() as session:
        q = await session.execute(sa_select(Game).filter(Game.id == game_id))
        game = q.scalars().first()
        if not game:
            raise HTTPException(status_code=404, detail="game not found")
        q2 = await session.execute(sa_select(Move).filter(Move.game_id==game_id).order_by(Move.id))
